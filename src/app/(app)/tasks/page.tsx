...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  MATCHING: { label: "匹配中", color: "text-yellow-400" },
  PENDING: { label: "待确认", color: "text-yellow-400" },
  EVALUATING: { label: "评估中", color: "text-blue-400" },
  ACCEPTED: { label: "已接单", color: "text-blue-400" },
  EXECUTING: { label: "执行中", color: "text-indigo-400" },
  COMPLETED: { label: "已完成", color: "text-emerald-400" },
  FAILED: { label: "失败", color: "text-red-400" },
  CANCELLED: { label: "已取消", color: "text-zinc-400" },
};
  ...
  return (
    <div className="min-h-screen bg-black pt-24 px-6">
      <div className="max-w-3xl mx-auto space-y-6">
        <h1 className="text-3xl font-bold text-white">我的任务</h1>
        ...
        <div className="flex gap-1 bg-zinc-900 rounded-lg p-1 w-fit">
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${
              tab === "published" ? "bg-white text-black" : "text-zinc-400 hover:text-white"
            }`}
          >
            我发布的
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${
              tab === "received" ? "bg-white text-black" : "text-zinc-400 hover:text-white"
            }`}
          >
            分身接的
          </button>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-zinc-500">暂无任务</div>
        ) : (
          <div className="space-y-3">
            {tasks.map((task) => {
              const statusInfo = STATUS_LABELS[task.status] || { label: task.status, color: "text-zinc-400" };
              return (
                <div
                  key={task.id}
                ...
                >
                  ...
                  {task.result && task.status === "COMPLETED" && (
                    <div className="mt-3 p-3 bg-zinc-800 rounded-lg text-sm text-zinc-300 max-h-24 overflow-hidden">
                      {task.resultUrl ? (
